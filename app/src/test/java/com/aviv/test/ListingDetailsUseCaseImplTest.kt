...
class ListingDetailsUseCaseImplTest {
    ...
    @Test
    fun `GIVEN repo returns success WHEN get details is called THEN onSuccess is invoked`() = runBlocking {
        // Given
        ...
        val listingId = 1
        coEvery { listingDetailsRepository.getListingDetails(listingId) } returns Resource.Success(mockListing)
    ...
    @Test
    fun `GIVEN repo returns error WHEN get details is called THEN onError is invoked`() = runBlocking {
        // Given
        val mockException = AppException.DefaultRemoteException("Network Error")
        val onSuccess: suspend (ListingsModel) -> Unit = mockk(relaxed = true)
        val onError: suspend (AppException) -> Unit = mockk(relaxed = true)
        val listingId = 1
        coEvery { listingDetailsRepository.getListingDetails(listingId) } returns Resource.Error(mockException)
